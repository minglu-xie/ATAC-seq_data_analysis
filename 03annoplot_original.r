#!/bin/bash
#################################################
#  File Name:03annoplot_original.r
#  Author: Minglu.Xie
#  Mail: minglu.xie.1023@student.uu.se,mingluxie@gmail.com
#  Created Time: Fri 03 Feb 2023 10:37:03 PM CET
#################################################


#Usage:
#/disk1/minglu/software/miniconda3/envs/r420/bin/Rscript annoplot_original.r -s mm -f KOR1.filterBL.bed -d KOR2.filterBL.bed -t NULLR1.filterBL.bed -i NULLR2.filterBL.bed
##Input files are from original four filterBL.bed files.

library("getopt")
spec<-matrix(c(
    "species", "s", "1", "character", "which species? hs, mm",
    "first", "f", "2", "character", "first bed file",
    "second", "d", "2", "character", "second bed file",
	"third", "t", "2", "character", "third bed file",
	"forth", "i", "2", "character", "forth bed file",
...
#header needs to be false,since the input is bed file without colnames.
up = read.table(opt$first,header = FALSE)
down = read.table(opt$second,header =FALSE)
r1 = read.table(opt$third,header =FALSE)
r2 = read.table(opt$forth,header =FALSE)
up_name<- gsub(".filterBL.bed","",opt$first)
down_name<- gsub(".filterBL.bed","",opt$second)
r1_name <- gsub(".filterBL.bed","",opt$third)
r2_name <- gsub(".filterBL.bed","",opt$forth)
...
bbb <- function(data1, name1)
{
	...
	### go anno###
	#single data plot#####
	#pdf(file=paste0(args[1],".annobar.pdf"),height =3, width = 6)
	#plotAnnoBar(data3)
	#dev.off()
	# data4 = as.data.frame(data3)
...
r1_anno = bbb(r1,r1_name)
r2_anno = bbb(r2,r2_name)

all = list(up =up_anno, down = down_anno, r1=r1_anno,r2=r2_anno)
names(all)<- c(up_name,down_name,r1_name,r2_name)
